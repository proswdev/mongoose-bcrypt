...
function createOneDocument(model, document, testSetter, done) {
  return new model(document).save(function (err, testDoc) {
...
function createOndeDocumentWithInsertMany(model, document, testSetter, done) {
 return model.insertMany([document], function (err, testDocs) {
   should.not.exist(err);
   testSetter(testDocs[0]);
   done();
 });
}
...
describe('mongoose-bcrypt', function() {
  ...
  describeSuites = [
    {
      case: 'create one document',
      creatingFunction: createOneDocument
    },
    {
      case: 'create many documents',
      creatingFunction: createOndeDocumentWithInsertMany
    }
  ]

  describeSuites.forEach(function(describeSuite) {
    var caseText = describeSuite.case;
    var creatingFunction = describeSuite.creatingFunction;

    describe(caseText, function() {
